...
class CNN(Model) :
    ...
    def build_model(model):
        ...
        x = Input(in_shape)
        conv2d = Conv2D(32, kernel_size=(3, 3), activation='relu', input_shape=in_shape)(x)
        conv2d = Conv2D(64, (3, 3), activation='relu')(conv2d)
        ...
        y = Dense(nb_classes, activation='softmax', name='preds')(dropout)
...
class Machine():
    ...
    def run(self, nb_epoch = 10, batch_size = 128, verbose = 1) :
        ...
        history = self.fit(epochs = nb_epoch, batch_size = batch_size, verbose = verbose)
